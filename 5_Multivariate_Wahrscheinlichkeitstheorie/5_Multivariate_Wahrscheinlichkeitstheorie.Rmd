...
knitr::include_graphics("5_Abbildungen/mvda_5_otto.png")
```
...
\center
\huge
\textcolor{black}{(5) Multivariate Wahrscheinlichkeitstheorie}
\vfill
...
knitr::include_graphics("5_Abbildungen/mvda_5_wahrscheinlichkeitstheorie_modell.pdf")
```

\small
\center

[\textcolor{darkblue}{BSc Psychologie | Grundlagen der Mathematik und Informatik}](https://bit.ly/3SNh3nR)

[\textcolor{darkblue}{BSc Psychologie | Wahrscheinlichkeitstheorie und Frequentistische Inferenz}](https://bit.ly/3yT42Sj)

# Vorbemerkungen
\textcolor{darkblue}{Realisierungen von Zufallsvariablen}
\footnotesize
\setstretch{1.1}
\vspace{2mm}
```{r}
...
# 10 Realisierungen
X       = rnorm(n,mu,sqrt(sigsqr))   # \xi_i \sim N(\mu,\sigma^2), i = 1,...,n
print(X)
...
x_min  = 0                                           # x Minimum
x_max  = 4                                           # x Maxim
x_res  = 1e3                                         # x Auflösung
x      = seq(x_min, x_max, length.out = x_res)       # x Raum

# Univariate Normalverteilungsparameter
mu     = 2                                           # Erwartungsparameter
sigsqr = 0.4                                         # Varianzparameter
...
dev.copy2pdf(                                                                    # export to PDF
             file   = file.path(fdir, "mvda_5_univariate_statistik.pdf"),        # filename
             width  = 8,                                                         # PDF width
             height = 4                                                          # PDF height
             )
...
knitr::include_graphics("5_Abbildungen/mvda_5_univariate_statistik.pdf")
```


# Vorbemerkungen
\textcolor{darkblue}{Realisierungen von Zufallsvektoren}
\footnotesize
\setstretch{1}
\vspace{1mm}
```{r}
...
Sigma   = matrix(c(0.5,0.1,          # Kovarianzmatrixparamter
                   0.1,0.5),
                 nrow = 2,
                 byrow = TRUE)
n       = 10                         # Anzahl von u.i.v. Realisierungen
...
# 10 Realisierungen
X       = t(mvrnorm(n,mu,Sigma))     # \xi_i \sim N(\mu,\Sigma), i = 1,...,n
print(X)


# 10 Realisierungen
X       = t(mvrnorm(n,mu,Sigma))     # \xi_i \sim N(\mu,\sigma^2), i = 1,...,n
print(X)
...
# Wahrscheinlichkeitsdichtefunke
contour(
x_1,
x_2,
...
# export to pdf
dev.copy2pdf(                                                                    # export to PDF
             file   = file.path(fdir, "mvda_5_multivariate_statistik.pdf"),     # filename
             width  = 8,                                                         # PDF width
             height = 4                                                          # PDF height
             )
...
```{r, echo = FALSE, out.width = "100%"}
knitr::include_graphics("5_Abbildungen/mvda_5_multivariate_statistik.pdf")
```
...
* Z.B. gilt beim Würfel für $\omega = 2$ mit $\omega \in \{2,4,6\}$, dass das Ereignis "Eine gerade Zahl fällt" eingetreten ist.
...
* Ohne Beweis halten wir fest, dass $\xi$ messbar ist, wenn die Funktionen $\xi_1,...,\xi_m$ messbar sind.
...
\vfill
```{r, echo = FALSE,out.width = "90%"}
knitr::include_graphics("5_Abbildungen/mvda_5_zufallsvektor.pdf")
```
\vfill
...
* Zum Beispiel heißt $x \le y$ für $x,y \in \mathbb{R}^m$, dass $x_i \le y_i$ für alle $i = 1,...,m$.
...
Man beachte, dass $\sum_{x \in \mathcal{X}} p_\xi(x) = \sum_{x_1 = 1}^3 \sum_{x_2 = 1}^4 p_\xi(x_1,x_2) = 1$.
...
* Für eine ausführliche Diskussion der Begriffe, siehe [\textcolor{darkblue}{Wahrscheinlichkeitstheorie und Frequentistische Inferenz}](https://bit.ly/3yT42Sj).
...
* Der Erwartungswert von $\xi$ ist der Vektor der Erwartungswerte $\mathbb{E}(\xi_1), ..., \mathbb{E}(\xi_m)$.
...
* Die Kovarianzmatrix $\mathbb{C}(\xi)$ ist die $m \times m$ Matrix der Kovarianzen $\mathbb{C}(\xi_i,\xi_j), i,j = 1,...,m$.
* Im ALM mit sphärischer Kovarianmatrix gilt per Definition $\mathbb{C}(\varepsilon) = \mathbb{C}(y) = \sigma^2 I_n \in \mathbb{R}^{n \times n}$.
...
* Es gelten $\rho_{ij} \in [-1,1]$ für $1 \le i,j \in m$ und $\rho_{ii} = 1$ für $1 \le i \le m$.
...
* Bei u.i.v. $\ups^{(1)},...,\ups^{(n)}$ ist $\bar{\ups}$ ein unverzerrter Schätzer von $\mathbb{E}(\ups^{(1)})$.
* Bei u.i.v. $\ups^{(1)},...,\ups^{(n)}$ ist $C_{\ups}$ ist ein unverzerrter Schätzer von $\mathbb{C}(\ups^{(1)})$.
...
fname = file.path(getwd(), "5_Multivariate_Wahrscheinlichkeitstheorie.csv")
write.csv(Y, file = fname, row.names = FALSE)
```
...
Die Darstellung des Stichprobenmittels ergibt sich nach Definition und mit ${\ups}^{(i)} = y^{(i)}$ aus
\begin{align}
\begin{split}
\bar{y} 
...
nach Definition und mit ${\ups}^{(i)} = y^{(i)}$ gilt, dass 
\begin{align}
\begin{split}
C_y  
...
Hinsichtlich der Korrelationsmatrix ergibt sich nach Definition und mit ${\ups}^{(i)} = y^{(i)}$
und für ein beliebiges Indexpaar $i,j$ mit $1 \le i,j \le m$, dass
\begin{align}
\begin{split}
R_{{y}_{ij}} 
